...
export default function DetailedCarbohydrate(props) {
    ...
    const addDataCarbohydrate = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.Carbohydrate,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Carbohydrate);
        });
    }
    const editDataCarbohydrate = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.Carbohydrate ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Carbohydrate);
        });
    ...
    const recordsDisplayer = data_Carbohydrate.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.Carbohydrate, id).then(r => {
            //update data
            updateHealthData(HealthDataType.Carbohydrate);
        })
    }
        ...
        <Container>
            ...
            <S_cart >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_Carbohydrate()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_Carbohydrate()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataCarbohydrate(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataCarbohydrate(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataCarbohydrate();setIndexOpenedRecordRow(-1)}}>
