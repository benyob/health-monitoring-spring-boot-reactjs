...
export default function DetailedSleepingHours(props) {
    ...
    const addDataSleepingHours = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.SleepingHours,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.SleepingHours);
        });
    }
    const editDataSleepingHours = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.SleepingHours ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.SleepingHours);
        });
    ...
    const recordsDisplayer = data_SleepingHours.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.SleepingHours, id).then(r => {
            //update data
            updateHealthData(HealthDataType.SleepingHours);
        })
    }
    ...
    return (
        <Container>
            ...
            <S_cart  >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ...
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_SleepingHours()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataSleepingHours(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataSleepingHours(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataSleepingHours();setIndexOpenedRecordRow(-1)}}>
