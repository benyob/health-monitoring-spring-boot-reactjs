...
export default function DetailedCalories(props) {
    ...
    const addDataCalories = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.Calories,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Calories);
        });
    }
    const editDataCalories = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.Calories ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Calories);
        });
    ...
    const recordsDisplayer = data_Calories.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.Calories, id).then(r => {
            //update data
            updateHealthData(HealthDataType.Calories);
        })
    }
        ...
        <Container>
            ...
            <S_cart >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_Calories()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_Calories()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataCalories(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataCalories(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataCalories();setIndexOpenedRecordRow(-1)}}>
