...
export default function DetailedProtein(props) {
    ...
    const addDataProtein = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.Protein,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Protein);
        });
    }
    const editDataProtein = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.Protein ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Protein);
        });
    ...
    const recordsDisplayer = data_Protein.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.Protein, id).then(r => {
            //update data
            updateHealthData(HealthDataType.Protein);
        })
    }
        ...
        <Container>
            ...
            <S_cart >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_Protein()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_Protein()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataProtein(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataProtein(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataProtein();setIndexOpenedRecordRow(-1)}}>
