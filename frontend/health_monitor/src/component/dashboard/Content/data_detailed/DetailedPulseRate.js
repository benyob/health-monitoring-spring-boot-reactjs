...
import ThemeService, { cols, colsGraph, themes, icons, colsRgba } from '../../../../service/theme.service';
import { HealthDataContex } from '../../../CompDashboard';

export default function DetailedPulseRate() {
    ...
    const addDataPulseRate = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.PulseRate,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.PulseRate);
        });
    }
    const editDataPulseRate = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.PulseRate ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.PulseRate);
        });
    ...
    const recordsDisplayer = data_PulseRate.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.PulseRate, id).then(r => {
            //update data
            updateHealthData(HealthDataType.PulseRate);
        })
    }
        ...
        <Container>
            ...
            <S_cart theme={theme} >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_PulseRate()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_PulseRate()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
                <Flag>
                    <p>{getText('Reference Value') + " :  60 - 100 BMP"}</p>
                    <div>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataPulseRate(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataPulseRate(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataPulseRate();setIndexOpenedRecordRow(-1)}}>
