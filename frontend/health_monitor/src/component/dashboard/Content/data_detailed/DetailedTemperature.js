...
export default function DetailedTemperature() {
    ...
    const addDataTemperature = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.Temperature,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Temperature);
        });
    }
    const editDataTemperature = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.Temperature ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Temperature);
        });
    ...
    const recordsDisplayer = data_Temperature.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.Temperature, id).then(r => {
            //update data
            updateHealthData(HealthDataType.Temperature);
        })
    }
    ...
    return (
        <Container>
            ...
            <S_cart theme={theme} >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ...
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_Temperature()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataTemperature(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataTemperature(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataTemperature();setIndexOpenedRecordRow(-1)}}>
