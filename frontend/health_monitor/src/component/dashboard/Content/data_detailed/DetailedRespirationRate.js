...
export default function DetailedRespirationRate() {
    ...
    const chartdata_RespirationRate = () => {
    ...
    const addDataRespirationRate = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.RespirationRate,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.RespirationRate);
        });
    }
    const editDataRespirationRate = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.RespirationRate ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.RespirationRate);
        });
    ...
    const recordsDisplayer = data_RespirationRate.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.RespirationRate, id).then(r => {
            //update data
            updateHealthData(HealthDataType.RespirationRate);
        })
    }
        ...
        <Container>
            ...
            <S_cart theme={theme} >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_RespirationRate()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_RespirationRate()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
                <Flag>
                    <p>{getText('Reference Value') + " :  12 - 20 BMP"}</p>
                    <div>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataRespirationRate(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataRespirationRate(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataRespirationRate();setIndexOpenedRecordRow(-1)}}>
