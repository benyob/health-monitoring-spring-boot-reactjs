...
export default function DetailedWeight(props) {
    ...
    const addDataWeight = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData(HealthDataType.Weight, placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Weight);
        });
    }
    const editDataWeight = () => {
        //update data to server
        const { value, value2, date, note } = placeHolders;
        healthDataService.updateHealthData(HealthDataType.Weight, { id: indexEditedRecordRow, value: value, value2: value2, date: date, note: note }).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Weight);
        });
    ...
    const recordsDisplayer = data_Weight.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.Weight, id).then(r => {
            //update data
            updateHealthData(HealthDataType.Weight);
        })
    }
    ...
    return (
        <Container>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataWeight(e)}>
                    ...
                    {indexEditedRecordRow === -1
                        ? <input type="submit" value={getText("Add")} onSubmit={(e) => addDataWeight(e)} />
                        : <div>
                            <button id="btn-edit-check" onClick={() => { editDataWeight(); setIndexOpenedRecordRow(-1) }}>
