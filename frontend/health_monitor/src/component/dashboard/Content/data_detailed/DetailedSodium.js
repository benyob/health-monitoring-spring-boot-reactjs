...
export default function DetailedSodium(props) {
    ...
    const addDataSodium = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.Sodium,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Sodium);
        });
    }
    const editDataSodium = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.Sodium ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.Sodium);
        });
    ...
    const recordsDisplayer = data_Sodium.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.Sodium, id).then(r => {
            //update data
            updateHealthData(HealthDataType.Sodium);
        })
    }
        ...
        <Container>
            ...
            <S_cart >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_Sodium()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_Sodium()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataSodium(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataSodium(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataSodium();setIndexOpenedRecordRow(-1)}}>
