...
export default function DetailedCholesterolLevels() {
    ...
    const chartdata_CholesterolLevels = () => {
    ...
    const addDataCholesterolLevels = (e) => {
        //add in global ddata
        e.preventDefault();
        //update here

        //add data to server
        healthDataService.setHealthData( HealthDataType.CholesterolLevels,placeHolders).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.CholesterolLevels);
        });
    }
    const editDataCholesterolLevels = () => {
        //update data to server
        const {value ,value2 ,date ,note} = placeHolders;
        healthDataService.updateHealthData(HealthDataType.CholesterolLevels ,{id:indexEditedRecordRow,value:value ,value2:value2 ,date:date ,note:note}).then(res => {
            //then get data from server
            updateHealthData(HealthDataType.CholesterolLevels);
        });
    ...
    const recordsDisplayer = data_CholesterolLevels.map(d => {
        indexColorRecords++;
    ...
    const handleDeleteRecord = (id) => {
        ...
        //delete
        healthDataService.deleteRecord(HealthDataType.CholesterolLevels, id).then(r => {
            //update data
            updateHealthData(HealthDataType.CholesterolLevels);
        })
    }
        ...
        <Container>
            ...
            <S_cart theme={theme} >
                <S_cart_body theme={theme}>
                    {chartType === 'line'
                        ? <Line data={chartdata_CholesterolLevels()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                    {chartType === 'bar'
                        ? <Bar data={chartdata_CholesterolLevels()}
                            options={{ maintainAspectRatio: false }}
                        />
                        : <></>}
                </S_cart_body>
                <Flag>
                    <p>{getText('Reference Value') + " : 6.66 -> 11.1 (mmol/l)"}</p>
                    <div>
            ...
            <Cont_TableRecord>
                <FormAddRecord onSubmit={(e) => addDataCholesterolLevels(e)}>
                    ...
                    ?<input type="submit" value={getText("Add")} onSubmit={(e) => addDataCholesterolLevels(e)} />
                    :<div>
                        <button id="btn-edit-check" onClick={()=>{editDataCholesterolLevels();setIndexOpenedRecordRow(-1)}}>
